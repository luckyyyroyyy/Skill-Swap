...
from models import User, Skill, SwapRequest, Review, Badge, UserBadge, Notification, ChatMessage
...
class TestMatchingAlgorithm:
    ...
    def test_match_score_calculation(self, test_user, test_user2):
        ...
        with app.app_context():
            ...
            score = calculate_match_score(test_user, test_user2)
            assert score > 0  # Should have positive score due to matching skills
    ...
    def test_find_matches(self, test_user, test_user2):
        ...
        with app.app_context():
            # Add skills
            skill1 = Skill(name="javascript", type="want", user_id=test_user.id)
            skill2 = Skill(name="javascript", type="offer", user_id=test_user2.id)
            db.session.add_all([skill1, skill2])
            db.session.commit()
            
            matches = find_matches(test_user)
