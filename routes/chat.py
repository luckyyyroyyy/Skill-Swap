...
def chat_view(swap_id):
    try:
        ...
        other_user = db.session.get(User, swap.receiver_id if current_user.id == swap.sender_id else swap.sender_id)
        if not other_user:
            abort(404)
        ...
        if form.validate_on_submit():
            try:
                message = ChatMessage(swap_id=swap.id, sender_id=current_user.id, message=form.content.data)
        ...
        messages = ChatMessage.query.filter_by(swap_id=swap.id).order_by(ChatMessage.created_at.asc()).all()
        for msg in messages:
            if msg.sender_id != current_user.id:
                msg.is_read = True
        db.session.commit()

        return render_template("chat.html", swap=swap, messages=messages, other_user=other_user, form=form)
    except Exception as e:
