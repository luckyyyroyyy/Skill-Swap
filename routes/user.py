...
def dashboard():
    ...
    received_requests = SwapRequest.query.filter_by(
        receiver_id=current_user.id
    ).filter(SwapRequest.status.in_(["pending", "accepted"])).all()
        ...
        try:
            ...
            if form.profile_pic.data:
                ...
                if mime_type not in allowed_mimes:
                    flash('Invalid file type! Please upload a valid JPEG or PNG image.', 'danger')
...
def add_skill():
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            logger.info(f"User {current_user.username} added skill: {new_skill.name}")
            flash(f"Skill added successfully! +{XP_ADD_SKILL} XP 🚀", "success")
        except Exception as e:
            db.session.rollback()
